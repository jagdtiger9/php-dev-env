...
if [ -n "$(ls -A /var/lib/mysql)" ]
then
    echo "Have been initialized"
else
    # initialize pinba engine and default table structure
    echo "INSTALL PLUGIN pinba SONAME 'libpinba_engine.so';" | TERM=dumb mysql --protocol=socket -uroot -p${MYSQL_ROOT_PASSWORD}
    echo "CREATE DATABASE pinba;" | TERM=dumb mysql --protocol=socket -uroot -p${MYSQL_ROOT_PASSWORD}
